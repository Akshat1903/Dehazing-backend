...
imagePath = str(settings.BASE_DIR) + "/dehazing_runner/test.jpg"
dehazed1imagePath = str(settings.BASE_DIR) + "/dehazing_runner/dehazed1.jpg"
dehazed2imagePath = str(settings.BASE_DIR) + "/dehazing_runner/dehazed2.jpg"

def clean_directory_video():
    if os.path.isfile(imagePath):
        os.remove(imagePath)
    if os.path.isfile(dehazed1imagePath):
        os.remove(dehazed1imagePath)
...
def main_runner():
    ...
    firebaseConfig = {
    "apiKey": "AIzaSyAQKJn8W_hZBZpr9Wga25QXNkFA4iEeVz4",
    ...
    "serviceAccount": str(settings.BASE_DIR) + "/dehazing_runner/serviceAccountKey.json",
    }
    ...
    img = cv2.imread(imagePath)  # 0-255
